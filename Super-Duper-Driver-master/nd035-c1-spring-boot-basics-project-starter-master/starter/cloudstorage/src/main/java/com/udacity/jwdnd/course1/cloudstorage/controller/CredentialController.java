...
//  Trang chủ
    ...
    public String getHomePage( Authentication authentication, @ModelAttribute("newFile") File newFile,
                               ...
                               @ModelAttribute("newNote") Note newNote, Model model)
    {
...
//  Thêm thông tin xác thực
    ...
    public String newCredential( Authentication authentication, @ModelAttribute("newFile") File newFile,
                                 @ModelAttribute("newCredential") Credential newCredential,
                                 @ModelAttribute("newNote") Note newNote, Model model)
    {
        String userName = authentication.getName();
        String newUrl = newCredential.getUrl();
        String credentialIdStr = String.valueOf(newCredential.getCredentialId());
        ...
        User user = userService.getUser(userName);
        model.addAttribute("credential", credentialService.getCredentialListings(user.getUserId()));
        model.addAttribute("encryptionService", encryptionService);
        model.addAttribute("result", "Thêm mới thông tin xác thực thành công");
...
//  Xóa thông tin xác thực
                                    ...
                                    @ModelAttribute("newNote") Note newNote, Model model)
    {
        ...
        model.addAttribute("credential", credentialService.getCredentialListings(user.getUserId()));
        model.addAttribute("encryptionService", encryptionService);
        model.addAttribute("result", "Xóa thông tin xác thực thành công");
